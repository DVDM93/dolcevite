    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex m-2 p-2">
                <a href="{{ route('admin.new_entries.index') }}" class="px-4 py-2 mb-2 text-3xl text-white font-extrabold bg-green-500 hover:bg-green-300 rounded-lg">
                    News Index
                </a>
            </div>
            ...
            <div class="m-2 p-2 bg-slate-100 rounded">
                <div class="space-y-8 divide-y divide-gray-200 w-1/2 mt-10">
                    <form method="POST" action="{{ route('admin.new_entries.update', $newEntry->id) }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="sm:col-span-6 mb-2">
                            <label for="name" class="block text-sm font-medium text-gray-700"> Nome </label>
                            <div class="mt-1">
                                <input type="text" id="name" name="name" value="{{ $newEntry->name }}"
                        ...
                        <div class="sm:col-span-6">
                            ...
                            <div>
                                <img src="{{ Storage::url($newEntry->image) }}" alt="" class="w-10 h-10">
                            </div>
                            <div class="mt-1">
                        ...
                        <div class="sm:col-span-6">
                            ...
                            <div class="mt-1">
                                <input type="number" min="0.00" max="10000.00" step="0.01" id="price" name="price" value="{{ $newEntry->price }}"
                                    class="block w-full appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out sm:text-sm sm:leading-5" />
                            </div>
                            @error('price')
                                <div class="text-sm text-red-400">{{ $message }}</div>
